...
from register import registers_main, row_data, register_control_by_CH341A
from usb_connect_ch341a_1 import MyUSB, MyUSBSlave, lib
from config import *

# 写入寄存器装饰器
def change_register(addr):
    """
    ...
    if enable_registers_new_addr:
        addr_chip = map_old_addr2new_addr[addr]
    else:
        addr_chip = addr
    ...
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            value1 = self.registers_map[map_addr2row[addr_chip]].hex_value()
            print('get register data: ' + str(hex(addr_chip)) + ' -> ' + value1)
            value = self.registers_map[map_addr2row[addr_chip]].dec_value()
            self.write_registers([addr_chip, int(value)])
            return a
...
class AncControlWindow(FilterDesignerWindow):
    ...
    ui_registers_init_signal = QtCore.pyqtSignal(bool)
    ...
    def __init__(self):
        ...
        self.init_usb()
        self.hidden_register_map_control()
    ...
    def set_default_registers(self):
        ...
        row10 = row_data.RowData(False, False, False, False, False, False, False, False)
        row11 = row_data.RowData(False, False, False, False, False, False, False, False)
        row12 = row_data.RowData(True, False, False, False, False, False, False, False)
        row13 = row_data.RowData(True, False, False, False, False, False, False, False)
    ...
    def control_signals_slots(self):
        ...
        self.btn_register_read_write.clicked.connect(self.register_read_write_dialog)
        ...
        self.radiobutton_start_detect.toggled.connect(self.pause_usb)  # 暂停/开始USB实时监测
        self.ui_pause_signal.connect(self.usb.ui_pause_slot)
        self.ui_pause_signal.connect(self.usb_slave.ui_pause_slot)
        ...
        self.usb_connected_signal.connect(self.usb.usb_connect_state_slot)
        self.usb_connected_signal.connect(self.usb_slave.usb_connect_state_slot)
        self.usb.signal.connect(self.read_registers_to_ui)  # 寄存器读取数值后更新UI
        self.usb.state_signal.connect(self.set_usb_master_state)  # 连接状态提示
        ...
        self.ui_registers_init_signal.connect(self.usb.ui_registers_init_slot)
        ...
        self.ui_registers_init_signal_slave.connect(self.usb_slave.ui_registers_init_slot)

        self.radiobutton_power_off.toggled.connect(self.chip_power_control)  # power off

        self.radiobutton_anc.toggled.connect(self.set_registers_data_eval)  # EVAL
    ...
    def signals_slot_connect_control(self, state=True):
        if state:
            ...
            self.radiobutton_anc.toggled.disconnect(self.set_registers_data_eval)  # EVAL
    ...
    def hidden_register_map_control(self):
        """根据config文件配置是否隐藏register map control功能"""
        if hidden_register_map:
            self.tabWidget_2.removeTab(2)
            self.btn_set_registers.setHidden(True)
    ...
    def registers_map_dialog(self):
        ...
        dialog.signal.connect(self.registers_dialog_control)
        dialog.exec_()

    def register_read_write_dialog(self):
        dialog2 = register_control_by_CH341A.MyCH341AControlWindow(self.usb_state, self.combo_chip_chose.currentIndex())
        dialog2.register_write_signal.connect(self.write_registers)
        dialog2.exec_()

    def registers_dialog_control(self, i):
        """双击registers_map对话框时，根据返回数据更新UI"""
        value = i[1]
        addr = map_row2addr[i[0]]
        self.write_registers([addr, value])

    def write_registers(self, data):
        """
        将registers_map数据写入OTP
        :param data: [addr, value]
        :return:
        """
        ...
        elif self.combo_chip_chose.currentIndex() == 2:
            if self.usb_state == '芯片已连接...':
                self.ui_changed_signal.emit(data)
            if self.usb_slave_state == 'slave芯片已连接...':
                self.ui_changed_signal_slave.emit(data)
    ...
    def set_usb_slave_state(self, state):
        if self.usb_slave_state != state:
            self.usb_slave_state = state
            self.usb_connect_toggled_adjust_ui()

    def set_usb_master_state(self, state):
        if self.usb_state != state:
            self.usb_state = state
            self.usb_connect_toggled_adjust_ui()

    def usb_connect_toggled_adjust_ui(self):
        if self.usb_state == 'USB已连接,检测芯片中...' and self.usb_slave_state != 'slave芯片已连接...':
            self.tab_5.setEnabled(False)
            self.statusbar.showMessage('USB已连接,检测芯片中...')
        elif self.usb_state == '芯片已连接...' and self.usb_slave_state == 'slave芯片已连接...':
            self.tab_5.setEnabled(True)
            self.combo_chip_chose.setEnabled(True)
            self.radiobutton_power_on.setChecked(True)
            self.label_74.setText('双芯片模式')
            self.statusbar.showMessage('双芯片连接成功')
        elif self.usb_state == 'USB已连接,检测芯片中...' and self.usb_slave_state == 'slave芯片已连接...':
            self.tab_5.setEnabled(False)
            self.combo_chip_chose.setEnabled(False)
            self.radiobutton_power_on.setChecked(True)
            self.label_74.setText('连接出错')
            self.statusbar.showMessage('单芯片连接，开启控制请反接SDA SCL')
        elif self.usb_state == '芯片已连接...' and self.usb_slave_state != 'slave芯片已连接...' :
            self.tab_5.setEnabled(True)
            self.combo_chip_chose.setEnabled(False)
            self.radiobutton_power_on.setChecked(True)
            self.label_74.setText('单芯片模式')
            self.statusbar.showMessage('单芯片连接成功')
        else:
            self.tab_5.setEnabled(True)
            self.statusbar.showMessage('USB未连接')
    ...
    def pause_usb(self):
        if self.radiobutton_start_detect.isChecked():
            self.ui_pause_signal.emit(False)
        else:
            self.ui_pause_signal.emit(True)
    ...
    # chip power control
    def chip_power_control(self):
        if not self.usb_state or self.usb_state == 'USB未连接...':
            return
        # 单芯片模式关机方案
        if self.radiobutton_power_off.isChecked():
            if self.usb_slave_state == 'slave芯片已连接...':
                self.usb.ui_power_off_queue.put('双芯片模式')
            else:
                self.usb.ui_power_off_queue.put('单芯片模式')
        elif self.radiobutton_power_on.isChecked():
            self.usb.ui_power_on_queue.put(True)
            ...
            if self.chip_burn_times == 3:
                ...
                if not self.registers_map[1].bit[0]:
                    ...
                    if enable_registers_new_addr:
                        ...
                        addr_chip = addr
                    self.write_registers(data=[addr_chip, value1])
    ...
    def save_registers(self):
        if self.registers_save_file:
            filename = QtWidgets.QFileDialog.getSaveFileName(self, 'save file', self.registers_save_file, 'xml Files (*.xml);;All Files (*)')
        else:
            filename = QtWidgets.QFileDialog.getSaveFileName(self, 'save file', '.', 'xml Files (*.xml);;All Files (*)')
        if filename[0]:
            self.registers_save_file = filename[0]
    ...
    def import_registers(self):
        if self.registers_save_file:
            filename = QtWidgets.QFileDialog.getOpenFileName(self, 'open file', self.registers_save_file, 'xml Files (*.xml);;All Files (*)')
        else:
    ...
    def reset_registers(self):
        ...
        if self.usb_state == '芯片已连接...':
            if self.combo_chip_chose.currentIndex() == 0:
                self.ui_registers_init_signal.emit(True)
            else:
                self.ui_registers_init_signal_slave.emit(True)
    ...
    def py_cmp_func(self, e):
        ...
        if e == 0:
            ...
            self.usb_connected_signal.emit(False)
            self.set_usb_master_state('USB未连接...')
        if e == 3:
            lib.USBIO_OpenDevice(0)
            lib.USBIO_SetStream(0, 0x80)
            self.usb_connected_signal.emit(True)
            self.set_usb_master_state('USB已连接,检测芯片中...')
...
class MySplashScreen(QtWidgets.QSplashScreen):
    ...
    def __init__(self):
        super(MySplashScreen, self).__init__(QtGui.QPixmap(r':/control/icon/start.png'))
